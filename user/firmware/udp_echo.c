...
__declspec(shared export mem) volatile uint64_t rx_buf_start;
__declspec(shared export mem) volatile struct ring_meta rx_meta, tx_meta;
...
void dma_packet(__mem40 void *addr, uint8_t dma_len) {
    ...
    DEBUG(0x22222222, rx_meta.tail >> 32, rx_meta.tail & 0xffffffff, dma_len);
    ...
    DEBUG(0x22222222, rx_meta.tail >> 32, rx_meta.tail & 0xffffffff, dma_len);
    DEBUG(0x22222222, rx_buf_start >> 32, rx_buf_start & 0xffffffff, dma_len);
}
...
main(void)
{
    ...
    __mem40 void *pkt_hdr;    /* The packet in the CTM */
    ...
    for (;;) {
        ...
        DEBUG(0x11111111, 0x11111111, 0x11111111, 0x11111111);
        pkt_hdr = receive_packet(&pkt_rxed, sizeof(pkt_rxed));
        DEBUG(0x99999999, 0x99999999, 0x99999999, 0x99999999);
        ...
        pkt_len = pkt_ctm_data_size(pkt_rxed.nbi_meta.pkt_info.len, PKT_NBI_OFFSET, PKT_CTM_SIZE_256);
        ...
        DEBUG(0x99999999, rx_meta.head & 0xffffffff, rx_meta.tail & 0xffffffff, pkt_len);
        first_dma_len = pkt_len;
        temp1 = rx_meta.tail + pkt_len;
        temp2 = rx_buf_start + rx_meta.len;
        DEBUG(0x33333333, temp1 >> 32, temp1 & 0xffffffff, 0);
        DEBUG(0x33333333, temp2 >> 32, temp2 & 0xffffffff, 0);

        if ((rx_meta.tail + (uint64_t)pkt_len) > (rx_buf_start + (uint64_t)rx_meta.len)) {
            DEBUG(0x44444444, (rx_meta.tail + pkt_len) & 0xffffffff, (rx_buf_start + rx_meta.len)  & 0xffffffff, 0);
            first_dma_len = rx_buf_start + rx_meta.len - rx_meta.tail;
        }

        dma_packet(pkt_hdr, first_dma_len);
        if (first_dma_len != pkt_len)
            dma_packet((unsigned long long)pkt_hdr + first_dma_len, pkt_len-first_dma_len);
