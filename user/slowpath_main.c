...
void configure_device(struct nfp_cpp* cpp)
{
    struct nfp_rtsym_table* symbol_table = nfp_rtsym_table_read(cpp);
...
void describe_pci_device(struct rte_pci_device* dev)
{
    fprintf(stderr,
        "PCI device: %u:%hhu:%hhu.%hhu\n",
        dev->addr.domain,
        dev->addr.bus,
        dev->addr.devid,
        dev->addr.function);

    for (int i = 0; i < (sizeof(dev->mem_resource)/sizeof(dev->mem_resource[0])); i++)
    {
        if (dev->mem_resource[i].phys_addr == 0)
            continue;

        fprintf(stderr,
            "BAR[%d]: VIRT [0x%p ~ 0x%p] PHY [0x%p]\n",
                i, dev->mem_resource[i].addr,
                dev->mem_resource[i].addr + dev->mem_resource[i].len,
                (void*) dev->mem_resource[i].phys_addr);
    }
}
